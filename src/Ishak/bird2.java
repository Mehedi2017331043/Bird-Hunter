...
public class bird2 extends javax.swing.JFrame {

    boolean click = false, dragging = false, blood = false;
    int b = 1, bullet = 5, d = 0, n = 3;
    int s = 1, seconds = 30, minute = 0;
    ...
    private void initComponents() {
        ...
        target.setText("Target: 20 birds");
        getContentPane().add(target);
        target.setBounds(180, 0, 300, 45);
        ...
        if (d != 1) {
            ...
            if (x >= a + 50 && x <= a + 135 && y >= b + 52 && y <= b + 82) {
                if (y > 360) {
                  ...
                  if (s - 1 >= 20) {
                    JOptionPane.showMessageDialog(this,"Congatulation You Completed The Game");
                    retry nl = new retry();
                    nl.setVisible(true);
                    retry.score1.setText(" You scored " + (s - 1));
                    this.dispose();
